		...
		var drObj = {
			...
			ribbonFunction:
			{
				...
				createRibbonA:function(){
					...
					drObj.ribbonFunction.createTopRibbon(date.hours+":"+(date.minutes<10?"0"+date.minutes:date.minutes)).appendTo($ribbonA);	// create bottom ribbon and append it to ribbon anchor
					drObj.ribbonFunction.createBottomRibbon(date.month).appendTo($ribbonA);	// create top ribbon and append it to ribbon anchor
				...
				createTailRibbon:function(){
					...
					//$tailLeftDiv.appendTo($tailDiv); //ACA
					...
					//$tailRightDiv.appendTo($tailDiv); //ACA 
					//Hack for IE.
				...
				getMonthAndDay:function(){
					var date = {month:drObj.monthNames[options.date.getMonth()],day:options.date.getDate(),hours:options.date.getHours(),minutes:options.date.getMinutes()};
					return date;
				},
